...
X-Ray Diffraction Computed Tomography (XRDCT, \ref{diff_tomo}, \cite{XRDCT}) is
one of the raster scanning experiments where a pencil beam is scanned onto a sample.
...
an area detector and savec as a stack of images.
Those images are azimuthal averaged into powder diffractions patterns with the
...
the sample position: translation and rotation.
The tomogram is finally reconstructed from the back-projection if the sinogram.  

\begin{figure}
\label{diff_tomo}
\begin{center}
\includegraphics[width=15cm]{Diffraction_imaging.pdf}
...
assembled to create a sinogram, which is computationnaly back-projected to build
the tomogram.}
\end{center}
\end{figure}
...
storage and data analysis workflow in use cannot cope with the pace imposed by
modern detector: data analysis \emph{is} the limiting factor for the whole experiment.
...
(Figure \ref{computer}) acts as a NFS server over RDMA for optimal performances
with the detector and is directly connected to the GPFS storage cluster.
...
\label{comuter}
\begin{center}
\includegraphics[width=15cm]{lid15agpu1.pdf}
\caption{Data-analysis computer with the main interconection and their
...
to devote the file reading and decompression to the processor.
...
of the file to read on the input queue and putting the decoded data to the
output queue after reading and decoding.
...
\label{pool}
\begin{center}
\includegraphics[width=15cm]{pool_of_worker.pdf}
\caption{Workflow for the data reduction of a XRDCT experiment using the pool
of worker pattern.}
\end{center}
\end{figure}
...
when processing the sample set of frames and variing the size of the
pool of readers:
